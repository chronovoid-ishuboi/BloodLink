...
| File | Purpose |
|---|---|
| `pom.xml` | Java 21 Maven build, JavaFX, MySQL, BCrypt, JUnit, Java-WebSocket, Tess4J, compiler, test, run, and setup plugins. |
...
| `src/main/java/com/bloodlink/server/PushServer.java` | Standalone Java-WebSocket server for real-time nudge delivery. | `Java-WebSocket` |
...
| `controller/RegisterController.java` | Donor/requester form switching, NID OCR integration, validation, and registration. | `register.fxml` |
| `controller/DonorDashboardController.java` | Eligibility, availability, matches, responses, history, notifications, and profile. Uses PushClient. | `donor_dashboard.fxml` |
| `controller/RequesterDashboardController.java` | Request creation, multi-donor matching, lifecycle actions/history, notifications, and profile. Uses PushClient. | `requester_dashboard.fxml` |
...
| `model/Donor.java` | Donor health, availability, donation count, badge behavior, reference hospital ID, profile photo. |
...
| `model/BloodRequest.java` | Complete emergency request record (includes units needed/fulfilled, hospital ID). |
...
| `model/Review.java` | Mutual 1-5 star review model. |
| `model/Hospital.java` | Geographic hospital location model. |
| `model/Role.java` | DONOR, REQUESTER, ADMIN. |
| `model/AvailabilityStatus.java` | Donor availability choices. |
| `model/Urgency.java` | NORMAL, URGENT, CRITICAL plus score/radius weight. |
| `model/RequestStatus.java` | Request lifecycle states (including PARTIALLY_FULFILLED). |
...
| `service/MatchingService.java` | Compatible donor filtering, scoring, sorting, Haversine distance cutoffs, and persistence. |
| `service/RequestService.java` | Request creation and multi-donor lifecycle façade. |
| `service/DonorService.java` | Donor availability and health update validation. |
| `service/ProfileService.java` | Contact/profile/hospital changes and password verification/update. |
...
| `service/LocationService.java` | Haversine distance calculations and hospital data serving. |
| `service/ReviewService.java` | Submission of reputation reviews and two-sided handshake management. |

## DAOs

| File | Purpose |
|---|---|
| `dao/UserDAO.java` | User lookup, role mapping, registration, profile/password updates, photo BLOB access, and account states. |
| `dao/DonorDAO.java` | Available donors, health/status updates, hospital references, and donation history. |
| `dao/RequestDAO.java` | Request CRUD, matching, partial/full fulfillment, cancellation, history, and transitions. |
| `dao/NotificationDAO.java` | Transaction-aware notification creation and inbox queries. |
| `dao/AdminDAO.java` | Dashboard metrics, charts, demand table, paginated user/request search, and paginated audit query. |
| `dao/AuditDAO.java` | Shared audit insert helper. |
| `dao/HospitalDAO.java` | Read-only access to geographic hospital reference data. |
| `dao/ReviewDAO.java` | Storage and retrieval of mutual star ratings and tags. |
| `dao/PagedResult.java` | Wrapper for paginated `LIMIT`/`OFFSET` queries (`items`, `totalCount`). |
...
| `util/DatabaseSetup.java` | Creates database/schema and idempotently seeds demo data (including handshakes). |
...
| `util/PushClient.java` | Real-time WebSocket listener that triggers UI updates. |
| `util/NidScanDialog.java` | Custom dialog integrating Tess4J OCR for registration form-fill. |
...
| `resources/com/bloodlink/view/register.fxml` | Adaptive donor/requester registration screen (with NID scan button). |
...
| `test/dao/AdminDAOTest.java` | Verifies paginated result limits and offset logic. |

## Traceability references

`docs-original/` contains the templates for these documentation files before Phase 2 location-based feature additions.
