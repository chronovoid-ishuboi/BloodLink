...
- Maven 3.9+
- MySQL Server 8+
- Tesseract OCR (for NID scanning)
...
## 6. Run the Push Server (Required for Real-time)

In a separate terminal, launch the standalone WebSocket push server:

```bash
mvn -q compile exec:java -Dexec.mainClass=com.bloodlink.server.PushServer
```

*(Note: The client will safely fall back to 10-second polling if this server is not running, but real-time nudges will be disabled).*
...
### `java.lang.UnsatisfiedLinkError` or `Unable to load library 'tesseract'`
Tess4J requires the Tesseract C++ binaries to be installed on your OS and available on your system `PATH`. Install `tesseract-ocr` via apt/brew/choco.

### `Location is not set` or FXML load failure
Run from the Maven project root. Confirm FXML files remain under `src/main/resources/com/bloodlink/view/` and that no double-hyphens (`--`) exist in XML comments.
